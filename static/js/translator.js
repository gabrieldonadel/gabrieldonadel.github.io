class Translator {
  ...
  async getTranslationByKey(lang, key) {
    ...
    if (typeof key != 'string')
      throw new Error(`Expected a string for the key parameter, got ${typeof key} instead.`)
  ...
  _getValueFromJSON(key, json, fallback) {
    ...
    if (!text && this._options.defaultLanguage && fallback) {
      let fallbackTranslation = JSON.parse(this._cache.get(this._options.defaultLanguage))
